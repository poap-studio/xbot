/**
 * Script to register Twitter webhook for TEST BRANCH
 * Usage: npx tsx scripts/register-webhook-testbranch.ts
 * 
 * NOTE: This registers the webhook for test.twitterbot.poap.studio (test environment)
 *       For production, use register-webhook.ts instead
 * 
 * ===================================================================================
 * IMPORTANT FINDINGS (2025-12-19):
 * ===================================================================================
 * 
 * 1. WEBHOOK LIMIT PER APP:
 *    Twitter limits webhooks to 1 per Developer App. If you already have a webhook
 *    registered (e.g., production), you cannot add another one for the same app.
 *    Error: { "message": "WebhookLimitExceeded: " }
 * 
 * 2. API VERSION:
 *    The v2 API (POST /2/webhooks) is correct and supports both:
 *    - BearerToken authentication (what this script uses)
 *    - UserToken (OAuth 1.0a) authentication
 *    Source: https://docs.x.com/x-api/webhooks/introduction (OpenAPI spec)
 * 
 * 3. OPTIONS FOR TEST ENVIRONMENT:
 *    a) Share the existing production webhook - both prod and test can use same webhook ID
 *    b) Use a different Twitter Developer App for test
 *    c) Temporarily swap production webhook URL (NOT RECOMMENDED)
 * 
 * 4. CURRENT PRODUCTION WEBHOOK:
 *    - URL: https://twitterbot.poap.studio/api/webhooks/twitter
 *    - ID: 1997725575138418689
 *    - Status: valid
 *    - Created: 2025-12-07
 * 
 * ===================================================================================
...
const WEBHOOK_URL = 'https://test.twitterbot.poap.studio/api/webhooks/twitter';
